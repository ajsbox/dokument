...
var _strDefaultSaveImageName = "___"+docId+"___"+table+"___s"+new Date().getTime();
function pageonload() {
...
function InitMessageBody() {
    ...
    if (MessageBody) {
        var ObjString = "<div class='divinput' style='line-height: 18px;background-color: #C1E5F0;border-radius: 4px;color: #30768D; display:none;'><b style='color: #1E586B;'>Note:</b> If you use other server-side language (JSP, PHP, ASP.NET (VB.NET), ASP etc.), you can visit the ";
        ObjString += "<a href='http://www.linuxsolutions.com.ve' target='_blank' >Linux Solutions, C.A.";
        ObjString += "</a>";
        ObjString += ".</div>";
...
function initMessageBox(bNeebBack) {
    ...
    objString += "<div style='position:relative; background:white; float:left; width:400px; height:35px;'>";
    ...
    objString += "<div style='position:relative; background:white; float:left; width:180px; height:35px;'>Modo de vista previa";
    ...
    objString += "<div><input id='DW_btnRemoveCurrentImage' onclick='btnRemoveCurrentImage_onclick()' type='button' value='Eliminar imágenes seleccionadas'/>";
    if (bNeebBack) {
        objString += "<input id='DW_btnRemoveAllImages' onclick='btnRemoveAllImages_onclick()' type='button' value='Eliminar todas las imágenes'/><br /><br />";
        objString += "<span style=\"font-size:larger\"><a href =\"online_demo_list.aspx\"><b>Back</b></a></span><br /></div>";
    }
    else {
        objString += "<input id='DW_btnRemoveAllImages' onclick='btnRemoveAllImages_onclick()' type='button' value='Eliminar todas las imágenes'/><br /></div>";
    }
    objString += "</div>";

    // The container for the error message
    objString += "<div id='DWTdivMsg' style='width:580px;display:inline'>";
    objString += "Mensajes del Sistema:<br />"
...
function initCustomScan() {
    ...
    ObjString += "<label id ='lblShowUI' for = 'ShowUI'><input type='checkbox' id='ShowUI' />Mostrar IU&nbsp;</label>";
    ...
    ObjString += "<li style='padding-left:15px;'>Tipo:";
    ObjString += "<label for='BW'><input type='radio' id='BW' name='PixelType'/>B&amp;N </label>";
    ObjString += "<label for='Gray'><input type='radio' id='Gray' name='PixelType'/>Gris</label>";
...
function setDefaultValue() {
    ...
    var varImgTypejpeg2 = document.getElementById("imgTypepdf2");
    if (varImgTypejpeg2)
        varImgTypejpeg2.checked = true;
    var varImgTypejpeg = document.getElementById("imgTypepdf");
    if (varImgTypejpeg)
   ...
   //var _strDefaultSaveImageName = "WebTWAINImage";
    ...
    _txtFileName.value = _strDefaultSaveImageName;
	 ...
	 var _txtFileName = document.getElementById("temp_holder");
    _txtFileName.value = _strDefaultSaveImageName;

	 var _chkMultiPagePDF_save = document.getElementById("MultiPagePDF");
    _chkMultiPagePDF_save.checked = true;
	var _chkMultiPagePDF_save = document.getElementById("Duplex");
    _chkMultiPagePDF_save.checked = true;
	
	var vResolution = document.getElementById("Resolution");
	vResolution.value = 200;
...
// Para realizar la descarga automatica
function setIp(){
	//document.cookie="serverIp=localhost";
}

function autoLoadImage(ip,path){	
	setTimeout(function(){ 
	DWObject.IfShowCancelDialogWhenImageTransfer = true;
	DWObject.IfShowProgressBar = true;
	DWObject.HTTPDownload("localhost", path, function(){console.log("Descarga Exitosa!");}, function(){console.log("Descarga fallida!");});	
	}, 
	3000);
...
function checkIfImagesInBuffer() {
    if (DWObject.HowManyImagesInBuffer == 0) {
        appendMessage("No hay imagenes en el buffer.<br />")
        return false;
    }
    else
        return true;
