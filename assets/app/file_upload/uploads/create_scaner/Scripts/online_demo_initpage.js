...
var _strDefaultSaveImageName = docId+"_cannedFiles_"+new Date().getTime();
function pageonload() {
...
function InitMessageBody() {
    ...
    if (MessageBody) {
        var ObjString = "<div class='divinput' style='line-height: 18px;background-color: #C1E5F0;border-radius: 4px;color: #30768D; display:none;'><b style='color: #1E586B;'>Note:</b> Si desea mas informacion vsite ";
...
function initMessageBox(bNeebBack) {
    ...
    /*objString += "<div style='text-align:center;margin-top:20px; width:580px; background-color:#FFFFFF;display:block'>";
    ...
    objString += "<br/>Mensajes de sistema:<br />"
...
function initCustomScan() {
    ...
    ObjString += "<label for='Resolution'>Resolución:<select size='1' id='Resolution'><option value = ''></option></select></label></li>";
    ObjString += "</ul>";
        ...
        if (DWObject.ErrorCode == 0) {                
            ...
            if (DWObject.SourceCount == 0) {
                if (Dynamsoft.Lib.env.bWin) {
                    ...
                    document.getElementById("aNoScanner").innerHTML = "<b>No se detectaron controladores TWAIN compatibles:<b/>";
...
function checkIfImagesInBuffer() {
    if (DWObject.HowManyImagesInBuffer == 0) {
        appendMessage("No hay imagenes el el buffer.<br />")
        return false;
    }
    else
        return true;
