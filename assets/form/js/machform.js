(function () {
    ...
    v.fn = v.prototype = {
        ...
        attr: function (c, d, e) {
            ...
            if (c.constructor == String)
                if (d == undefined) return this.length && v[e || "attr"](this[0], c) || undefined;
                else {
                    f = {};
                    f[c] = d
                }
        ...
        clone: function (d) {
            ...
            if (d === true) this.find("*")
                ...
                .each(function (i) {
                    ...
                    for (var b in a)
                        for (var c in a[b]) v.event.add(f[i], b, a[b][c], a[b][c].data)
                });
            return e
        },
        filter: function (t) {
    ...
    function evalScript(i, a) {
        ...
        if (b) {
            ...
            else v.globalEval(a.text || a.textContent || a.innerHTML || ""); if (a.parentNode) a.parentNode.removeChild(a)
    ...
    v.extend({
                ...
                if (typeof c == "string") {
                    ...
                    if (v.browser.msie) {
                        ...
                        for (var n = tb.length - 1; n >= 0; --n)
                            if (v.nodeName(tb[n], "tbody") && !tb[n].childNodes.length) tb[n].parentNode.removeChild(tb[n]);
        ...
        grep: function (a, b, c) {
            ...
            for (var i = 0, el = a.length; i < el; i++)
                if (!c && b(a[i], i) || c && !b(a[i], i)) d.push(a[i]);
            return d
        },
        map: function (a, b) {
                ...
                if (m) {
                    ...
                    for (var i = 0; b[i]; i++)
                        for (var c = b[i].firstChild; c; c = c.nextSibling)
                            if (c.nodeType == 1 && (f == "*" || c.nodeName.toUpperCase() == f.toUpperCase())) r.push(c);
                    b = r;
                    t = t.replace(e, "");
            ...
            if (d) {
                ...
                if (!b) {
                    ...
                    if (c) delete d[b][c.guid];
                        ...
                        for (c in d[b])
                            if (!e[1] || d[b][c].type == e[1]) delete d[b][c]; for (ret in d[b]) break;
                    if (!ret) {
    ...
    v.extend({
        ...
        param: function (a) {
            ...
            if (a.constructor == Array || a.jquery) v.each(a, function () {
                ...
                for (var j in a)
                    if (a[j] && a[j].constructor == Array) v.each(a[j], function () {
            ...
            if (t > this.options.duration + this.startTime) {
                ...
                if (a) {
                    ...
                    if (this.options.hide || this.options.show)
                        for (var p in this.options.curAnim) v.attr(this.elem.style, p, this.options.orig[p])
                }
...
(function ($) {
    ...
    $.fn.extend({
        height: function () {
            if (!this[0]) f();
            if (this[0] == window)
                if ($.browser.opera || ($.browser.safari && parseInt($.browser.version) > 520)) return self.innerHeight - (($(document)
                    .height() > self.innerHeight) ? j() : 0);
                else if ($.browser.safari) return self.innerHeight;
            else return $.boxModel && document.documentElement.clientHeight || document.body.clientHeight; if (this[0] == document) return Math.max(($.boxModel && document.documentElement.scrollHeight || document.body.scrollHeight), document.body.offsetHeight);
            return e.apply(this, arguments)
        },
        width: function () {
            if (!this[0]) f();
            if (this[0] == window)
                if ($.browser.opera || ($.browser.safari && parseInt($.browser.version) > 520)) return self.innerWidth - (($(document)
                    .width() > self.innerWidth) ? j() : 0);
                else if ($.browser.safari) return self.innerWidth;
            else return $.boxModel && document.documentElement.clientWidth || document.body.clientWidth; if (this[0] == document)
                if ($.browser.mozilla) {
...
element_properties.prototype = {
    ...
    choices: function () {
        ...
        for (var i = 0; i < options.length; i++) {
            ...
            all_markup[i] = "<li>" + "<input id=\"choice" + i + "\" class=\"text\" type=\"text\" maxlength=\"250\" autocomplete=\"off\" value=\"" + el_val + "\" " + "onkeyup=\"set_properties(this.value, 'choices', " + i + ")\" " + "onkeypress=\"choices_event(event," + i + ")\" />" + " <img class=\"button\" src='"+server_from_path+"images/icons/add.gif' alt=\"Agregar\" title=\"Agregar\" onclick=\"insert_choice(" + (i + 1) + ")\" /> " + "<img class=\"button\"  src='"+server_from_path+"images/icons/delete.gif' alt=\"Borrar\" title=\"Borrar\" onclick=\"delete_checkbox(" + options[i].id + ", " + options[i].is_db_live + ", '" + field_type + "', " + i + ")\" /> " + "<img class=\"button\" src='"+server_from_path+"images/icons/stardim.gif' alt=\"" + msg + "\" title=\"" + msg + "\" onclick=\"set_choice_default(" + i + ")\" />" + "</li>"
        }
        JJ("#element_choices")
...
field.prototype = {
    ...
    generate_markup: function () {
        this.li.innerHTML = "<img id=\"arrow\" src='"+server_from_path+"images/icons/arrow.gif' alt=\"\" class=\"arrow\"><a href=\"#\" class=\"hover_ready\" onclick=\"return false;\" title=\"Haga clic para editar. Arrastre para reordenar.\">" + this.field_label() + this[ds.get(this.id, "type")]() + "</a>" + this.element_actions()
    },
    ...
    element_actions: function () {
        ...
        return "<div class=\"element_actions\" " + style + "><img src='"+server_from_path+"images/icons/duplicate.gif' alt=\"Duplicar\" title=\"Duplicar\" onclick=\"duplicate_element(" + this.id + ")\"> <img src='"+server_from_path+"images/icons/delete.gif' alt=\"Borrar.\" title=\"Borrar\" onclick=\"delete_element(" + this.id + ")\"></div>"
    },
    text: function () {
    ...
    date: function () {
        return "<div><span><input readonly=\"readonly\" class=\"text\" size=\"2\" type=\"text\"> / <label>MM</label></span><span><input readonly=\"readonly\" class=\"text\" size=\"2\" type=\"text\"> / <label>DD</label></span><span><input readonly=\"readonly\" class=\"text\" size=\"4\" type=\"text\"> <label>YYYY</label></span><img src='"+server_from_path+"images/icons/calendar.gif' alt=\"Pick Date.\" class=\"icon\"></div>"
    },
    europe_date: function () {
        return "<div><span><input readonly=\"readonly\" class=\"text\" size=\"2\" type=\"text\"> / <label>DD</label></span><span><input readonly=\"readonly\" class=\"text\" size=\"2\" type=\"text\"> / <label>MM</label></span><span><input readonly=\"readonly\" class=\"text\" size=\"4\" type=\"text\"> <label>YYYY</label></span><img src='"+server_from_path+"images/icons/calendar.gif' alt=\"Pick Date.\" class=\"icon\"></div>"
    },
    time: function () {
...
form.prototype = {
    initialize: function () {
        this.data = {
            ...
            description: "Esta es la descripción de su forma. Haga clic aquí para editar.",
            redirect: "",
            success_message: " Su presentación se ha guardado!",
            password: "",
            frame_height: "",
            unique_ip: "0",
            captcha: "0"
    ...
    display: function () {
        ...
        JJ(this.li)
            .attr("id", "active_form")
            .addClass("info");
        this.li.innerHTML = "<img src='"+server_from_path+"images/icons/arrow.gif' alt=\"\" class=\"arrow\"><a href=\"#\" onclick=\"return false;\" class=\"hover_ready\" title=\"Click to edit.\"><h2 id=\"form_name\">" + this.data["name"] + "</h2><p id=\"form_desc\">" + this.data["description"].replace(/\n/g, "<br />") + "</p></a>";
        return this.li
    },
    selected: function () {
        JJ(this.li)
...
function save_form_data() {
    ...
    JJ("#form_save_button")
        .html("<img src='"+server_from_path+"images/icons/filesave.gif' /> Saving");
    main_form.data["frame_height"] = JJ("#main")
        .outerHeight();
    JJ("#form_save_button")
        .unbind("click");
    JJ.post(server_path+"document/update_form", {
...
function form_aftersave_callback(a) {
    ...
    JJ("#form_save_button")
        .html("<img src='"+server_from_path+"images/icons/filesave.gif' /> Guardar Formulario");
    JJ("#form_save_button")
        .click(function () {
            save_form_data()
        });
